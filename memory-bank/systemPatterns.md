...
**Projektanfragen (public.project_requests):**
Die Tabelle speichert Projektanfragen direkt aus dem Wizard. Struktur:
- Basis-Felder: `id`, `name`, `first_name`, `last_name`, `email`, `phone`, `company`
- Projekt-Daten: `project_type`, `budget`, `final_price`
- Projekt-Details: `timeline`, `priority`, `description`, `features` (TEXT[])
- Quelle: `source` (Label), `lead_source` (Enum)
- Einwilligungen: `privacy_accepted`, `marketing_accepted` (BOOLEAN)
- Analyse: `ai_analysis` (TEXT)
- CRM-Erweiterung: Adresse, Angebot, Vertrag, Follow-up, Stundensatz
- Metadaten: `status`, `notes`, `created_at`, `updated_at`

**Wichtig:** Die Tabelle verwendet nur Label-Felder (z.B. `timeline`, `priority`, `source`, `project_type`), keine separaten ID-Felder.
